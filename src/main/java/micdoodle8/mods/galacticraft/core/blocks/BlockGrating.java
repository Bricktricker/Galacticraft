...
public class BlockGrating extends Block implements ISortableBlock, IPartialSealableBlock
{
    ...
    private ThreadLocal<Boolean> replaceWithFluidIntended = ThreadLocal.withInitial(() -> false);
    ...
    {
        return this.getDefaultState().withProperty(BlockLiquid.LEVEL, meta).withProperty(BlockFluidBase.LEVEL, meta);
    }
    ...
    public void breakBlock(World world, BlockPos pos, IBlockState state)
    {
        ...
        if (newBlock.getBlock() instanceof BlockFluidBase)
        {
            for (BlockGrating b : forgeBlocks)
            {
                ...
                {
                    int level = newBlock.getValue(BlockFluidBase.LEVEL);
        ...
        if (newBlock.getBlock() instanceof BlockLiquid)
        {
            ...
            {
                int level = newBlock.getValue(BlockLiquid.LEVEL);
    ...
    public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer, EnumHand hand)
    {
        ...
        if (oldBlock.getBlock() instanceof BlockFluidBase)
        {
            for (BlockGrating b : forgeBlocks)
            {
                ...
                {
                    int level = oldBlock.getValue(BlockFluidBase.LEVEL);
        ...
        } else if (oldBlock.getBlock() instanceof BlockLiquid)
        {
            ...
            {
                int level = oldBlock.getValue(BlockLiquid.LEVEL);
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        ...
        int i = state.getValue(BlockLiquid.LEVEL);
        int j = 1;
        ...
        if (i > 0)
        {
            ...
            if (this.adjacentSourceBlocks >= 2 && net.minecraftforge.event.ForgeEventFactory.canCreateFluidSource(worldIn, pos, state, this.blockMaterial == Material.WATER))
            {
                ...
                {
                    i1 = 0;
                } else if (iblockstate.getMaterial() == this.blockMaterial && iblockstate.getValue(BlockLiquid.LEVEL) == 0)
                {
                    i1 = 0;
                }
            }
...
//                this.placeStaticBlock(worldIn, pos, state);
            } else
            {
                ...
                {
                    state = state.withProperty(BlockLiquid.LEVEL, i1);
...
//            this.placeStaticBlock(worldIn, pos, state);
    ...
    private void tryFlowInto(World worldIn, BlockPos pos, IBlockState state, int level)
    {
        if (this.canFlowInto(worldIn, pos, state))
        {
            ...
            worldIn.setBlockState(pos, this.liquidEquivalentDynamic.withProperty(BlockLiquid.LEVEL, level), 3);
        }
    }
    ...
    private int getSlopeDistance(World worldIn, BlockPos pos, int distance, EnumFacing calculateFlowCost)
    {
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
        {
            if (enumfacing != calculateFlowCost)
            {
                ...
                if (!this.isBlocked(worldIn, blockpos, iblockstate) && (iblockstate.getMaterial() != this.blockMaterial || iblockstate.getValue(BlockLiquid.LEVEL) > 0))
                {
    ...
    private boolean isBlocked(World worldIn, BlockPos pos, IBlockState state)
    {
        ...
        return block instanceof BlockDoor || block == Blocks.STANDING_SIGN || block == Blocks.LADDER || block == Blocks.REEDS || (block.getMaterial(bs) == Material.PORTAL || block.getMaterial(bs) == Material.STRUCTURE_VOID || block.getMaterial(bs).blocksMovement());
    }
    ...
    {
        return p_189542_1_.getMaterial() == this.blockMaterial ? p_189542_1_.getValue(BlockLiquid.LEVEL) : -1;
    }
