...
{
    public static final PropertyEnum<GlassModel> MODEL = PropertyEnum.create("modeltype", GlassModel.class);
    ...
    {
        return new BlockStateContainer(this, MODEL, ROTATION);
    }
    ...
    protected boolean buildSolidSideUD(BlockPos pos, EnumFacing testUD, IBlockAccess worldIn, IBlockState blockState)
    {
        ...
        return (trueThis & 14) != (trueOther & 14);
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos)
    {
        ...
        if (connections == 0)
        {
            ...
            if (below.getBlock() != this || !this.isConnectedNSRecursive(below, worldIn, pos.down(), EnumFacing.DOWN))
            {
                if (above.getBlock() != this || !this.isConnectedNSRecursive(above, worldIn, pos.up(), EnumFacing.UP))
                {
                    if (this.isPreferenceEW(state, worldIn, pos))
                    {
                        rot = GlassRotation.E;
...
//        }
    ...
    private IBlockState getModel(IBlockState state, int model, GlassRotation rot, boolean plateD, boolean plateL, boolean plateR, boolean plateU, int cornerPiece)
    {
        ...
        switch (model)
        {
            case 1:
                if (solids == 4)
                    return state.withProperty(MODEL, GlassModel.STANDARD_S4).withProperty(ROTATION, GlassRotation.get(y, 0));
                ...
                if (solids == 3)
                {
                    ...
                    else if (!plateR)
                    {
                        ...
                        x = (y % 2 == 1) ? 1 : 3;
                    return state.withProperty(MODEL, GlassModel.STANDARD_S3).withProperty(ROTATION, GlassRotation.get(y, x));
                }

                if (plateD && plateU)
                    return state.withProperty(MODEL, GlassModel.STANDARD_S2A).withProperty(ROTATION, GlassRotation.get(y, 0));

                if (plateL && plateR)
                    return state.withProperty(MODEL, GlassModel.STANDARD_S2A).withProperty(ROTATION, GlassRotation.get(y, 1));

                if (plateD && (plateR || plateL))
                {
                    return state.withProperty(MODEL, cornerPiece > 0 ? GlassModel.STANDARD_S2B : GlassModel.STANDARD_S2).withProperty(ROTATION, GlassRotation.get(plateL ? (y ^ 2) : y, 0));
                }

                if (plateU && (plateR || plateL))
                {
                    return state.withProperty(MODEL, cornerPiece > 0 ? GlassModel.STANDARD_S2B : GlassModel.STANDARD_S2).withProperty(ROTATION, GlassRotation.get(plateL ? (y ^ 2) : y, 2));
                }

                if (plateU || plateD)
                    return state.withProperty(MODEL, getCornerModel(cornerPiece)).withProperty(ROTATION, GlassRotation.get(y, (plateU || cornerPiece > 4) ? 2 : 0));

                if (plateR || plateL)
                    return state.withProperty(MODEL, getCornerModel(cornerPiece)).withProperty(ROTATION, GlassRotation.get(plateL ? (y ^ 2) : y, 1 + (cornerPiece > 4 ? 2 : 0)));

                return state.withProperty(MODEL, GlassModel.STANDARD_PANE).withProperty(ROTATION, GlassRotation.get(y, 0));
            case 2:
                if (plateU && plateD)
                    return state.withProperty(MODEL, GlassModel.CORNER_S2).withProperty(ROTATION, GlassRotation.get(y, 0));
                if (plateD)
                    return state.withProperty(MODEL, GlassModel.CORNER_S).withProperty(ROTATION, GlassRotation.get(y, 0));
                if (plateU)
                    return state.withProperty(MODEL, GlassModel.CORNER_S).withProperty(ROTATION, GlassRotation.get((y + 3) % 4, 2));

                return state.withProperty(MODEL, GlassModel.CORNER).withProperty(ROTATION, GlassRotation.get(y, 0));
            case 3:
                if (plateD && plateU)
                    return state.withProperty(MODEL, GlassModel.T_JUNCTION_S2).withProperty(ROTATION, GlassRotation.get(y, 0));
                if (plateD)
                    return state.withProperty(MODEL, GlassModel.T_JUNCTION_S).withProperty(ROTATION, GlassRotation.get(y, 0));
                if (plateU)
                    return state.withProperty(MODEL, GlassModel.T_JUNCTION_S).withProperty(ROTATION, GlassRotation.get(y ^ 2, 2));

                return state.withProperty(MODEL, GlassModel.T_JUNCTION).withProperty(ROTATION, GlassRotation.get(y, x));
            case 4:
                if (plateD && plateU)
                    return state.withProperty(MODEL, GlassModel.CROSSROADS_S2).withProperty(ROTATION, GlassRotation.get(y, 0));
                if (plateD)
                    return state.withProperty(MODEL, GlassModel.CROSSROADS_S).withProperty(ROTATION, GlassRotation.get(y, 0));
                if (plateU)
                    return state.withProperty(MODEL, GlassModel.CROSSROADS_S).withProperty(ROTATION, GlassRotation.get(y ^ 2, 2));
            default:
                return state.withProperty(MODEL, GlassModel.CROSSROADS).withProperty(ROTATION, GlassRotation.get(y, x));
        }
    }

    private GlassModel getCornerModel(int cornerPiece)
    {
        if ((cornerPiece & 2) == 2)
    ...
    public enum GlassFrame implements IStringSerializable
    {
        ...
        GlassFrame(String name, int color)
        {
            this.name = name;
