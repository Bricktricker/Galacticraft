...
import micdoodle8.mods.galacticraft.api.vector.MatrixUtils;
...
public class GuiCelestialSelection extends Screen
{
    ...
    protected static final ResourceLocation guiMain0 = new ResourceLocation(Constants.MOD_ID_CORE, "textures/gui/celestialselection.png");
    protected static final ResourceLocation guiMain1 = new ResourceLocation(Constants.MOD_ID_CORE, "textures/gui/celestialselection1.png");
    ...
    protected final Map<CelestialBody, Vector3> planetPosMap = Maps.newHashMap();
    @Deprecated
    ...
    protected final Vector2 translation = new Vector2(0.0F, 0.0F);
    ...
    protected final List<CelestialBody> bodiesToRender = Lists.newArrayList();
...
//        } TODO Celestial zoom
        ...
        MatrixUtils.translate(new Vector3(0.0F, 0.0F, -9000.0F), camMatrix, camMatrix); // See EntityRenderer.java:setupOverlayRendering
        ...
        for (Map.Entry<CelestialBody, Matrix4f> e : matrixMap.entrySet())
        {
            ...
            Matrix4f matrix0 = MatrixUtils.mul(viewMatrix, planetMatrix, planetMatrix);
            ...
            Vector4 newVec = MatrixUtils.transform(scaleVec, new Vector4(2, -2, 0, 0), null);
        ...
        fb.rewind();
        MatrixUtils.store(camMatrix, fb);
        fb.flip();
        fb.clear();
...
//            RenderSystem.color4f(5 * brightness, 0.6F + 2 * brightness, 1.0F - 4 * brightness, scale / 95.0F);
    ...
    public Matrix4f setIsometric(float partialTicks)
    {
        ...
        MatrixUtils.translate(new Vector3(width / 2.0F, height / 2, 0), mat0, mat0);
        MatrixUtils.rotate((float) Math.toRadians(55), new Vector3(1, 0, 0), mat0, mat0);
        MatrixUtils.rotate((float) Math.toRadians(-45), new Vector3(0, 0, 1), mat0, mat0);
        float zoomLocal = this.getZoomAdvanced();
        this.zoom = zoomLocal;
        MatrixUtils.scale(new Vector3(1.1f + zoomLocal, 1.1F + zoomLocal, 1.1F + zoomLocal), mat0, mat0);
        Vector2 cBodyPos = this.getTranslationAdvanced(partialTicks);
        this.position = this.getTranslationAdvanced(partialTicks);
        MatrixUtils.translate(new Vector3(-cBodyPos.x, -cBodyPos.y, 0), mat0, mat0);
    ...
    {
        Matrix4f worldMatrix0 = new Matrix4f(worldMatrix);
        MatrixUtils.translate(this.getCelestialBodyPosition(body), worldMatrix0, worldMatrix0);
        Matrix4f worldMatrix1 = new Matrix4f();
        MatrixUtils.rotate((float) Math.toRadians(45), new Vector3(0, 0, 1), worldMatrix1, worldMatrix1);
        MatrixUtils.rotate((float) Math.toRadians(-55), new Vector3(1, 0, 0), worldMatrix1, worldMatrix1);
        if (scaleXZ != 1.0F)
        {
            MatrixUtils.scale(new Vector3(scaleXZ, scaleXZ, 1.0F), worldMatrix1, worldMatrix1);
        }
        MatrixUtils.mul(worldMatrix0, worldMatrix1, worldMatrix1);
        fb.rewind();
