...
public class TileEntityPlatform extends TileEntity implements ITickable
{
    ...
    private boolean checkIntact()
    {
        ...
        if (bs.getBlock() != GCBlocks.platform || bs.getValue(BlockPlatform.CORNER).getMeta() != this.corner)
        {
            this.resetBlocks();
            return false;
        }
    ...
    private int checkState(BlockPos blockPos, int meta)
    {
        ...
        if (bs.getBlock() == GCBlocks.platform && bs.getValue(BlockPlatform.CORNER).getMeta() == meta)
        {
