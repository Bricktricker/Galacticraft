...
public class FluidUtil
{
    ...
    public static boolean isFuelContainerAny(ItemStack var4)
    {
        ...
        {
            return var4.getItem() == GCItems.FUEL_CANISTER.get() && var4.getDamage() < var4.getMaxDamage();
        }
...
//        return false;
    ...
    public static boolean isOilContainerAny(ItemStack var4)
    {
        if (var4.getItem() instanceof ItemCanisterGeneric)
        {
            return var4.getItem() == GCItems.OIL_CANISTER.get() && var4.getDamage() < var4.getMaxDamage();
        }
...
//        } TODO Bucket
        ...
        if (FluidUtil.isValidContainer(inventory.get(slot)))
        {
            ...
            if (!liquid.isEmpty() && liquid.getAmount() > 0)
            {
                ...
                if (liquidname.startsWith("fuel"))
                {
                    ...
                    FluidUtil.tryFillContainer(tank, liquid, inventory, slot, GCItems.FUEL_CANISTER.get());
                }
            }
        }
    }
    ...
    public static void loadFromContainer(FluidTank tank, Fluid desiredLiquid, NonNullList<ItemStack> stacks, int slot, int amountOffered)
    {
        ...
        if (slotItem.getItem() instanceof ItemCanisterGeneric)
        {
            ...
            int used = tank.fill(new FluidStack(desiredLiquid, ItemCanisterGeneric.EMPTY - originalDamage), IFluidHandler.FluidAction.EXECUTE);
            if (originalDamage + used >= ItemCanisterGeneric.EMPTY)
            {
                ItemStack item = new ItemStack(GCItems.OIL_CANISTER.get(), 1);
...
//        } TODO Bucket
    ...
    public static boolean isPartialContainer(ItemStack var4, Item canisterType)
    {
        if (var4.getItem() instanceof ItemCanisterGeneric)
        {
            return var4.getDamage() == ItemCanisterGeneric.EMPTY || (var4.getItem() == canisterType && var4.getDamage() > 1);
        }
    ...
    public static boolean isEmptyContainerFor(ItemStack var4, FluidStack targetFluid)
    {
        if (var4.getItem() instanceof ItemCanisterGeneric)
        {
            if (var4.getDamage() == ItemCanisterGeneric.EMPTY)
            {
                return true;
            }
            if (var4.getDamage() == 1)
