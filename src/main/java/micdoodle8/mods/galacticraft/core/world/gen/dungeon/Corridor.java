                ...
                for (int k = 0; k < this.boundingBox.getZSize(); k++)
                {
                    ...
                    else
                    {
                        if (j == this.boundingBox.getYSize() - 2)
                        {
                            ...
                            {
                                this.setBlockState(worldIn, GCBlocks.UNLIT_TORCH.get().getDefaultState().with(BlockUnlitTorch.FACING, i == 1 ? Direction.WEST.getOpposite() : Direction.EAST.getOpposite()), i, j, k, this.boundingBox);
                                continue;
                            }
                            else if (this.getDirection().getAxis() == Direction.Axis.X && (i + 1) % 4 == 0 && (k == 1 || k == this.boundingBox.getZSize() - 2))
                            {
                                this.setBlockState(worldIn, GCBlocks.UNLIT_TORCH.get().getDefaultState().with(BlockUnlitTorch.FACING, k == 1 ? Direction.NORTH.getOpposite() : Direction.SOUTH.getOpposite()), i, j, k, this.boundingBox);
                                continue;
                            }
                        }
